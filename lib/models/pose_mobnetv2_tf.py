...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    if is_train and cfg.MODEL.INIT_WEIGHTS:
        base_model.load_weights(cfg.MODEL.PRETRAINED)

    x = base_model.output
    if 0 < cfg.MODEL.EXTRA.NUM_DECONV_LAYERS:
        x = make_deconv_layer(
    ...
    if cfg.TRAIN.FINE_TUNING == "fixed_param":
        for layer in model.layers:
            if cfg.TRAIN.BORDER_NAME in layer.name:
                break
